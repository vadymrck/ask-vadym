...
def test_chat_redirects_offtopic_questions(test_client: TestClient):
    ...
    has_decline = any(
        indicator in message_lower for indicator in decline_indicators
    )
    assert (
        has_decline
...
def test_chat_handles_recent_experience_follow_up(test_client: TestClient):
    ...
    first_question = "What is your most recent experience?"
    ...
    second_question = "And before that?"
    second_response = ask_question_with_history(
        test_client, second_question, history
    )
